...
export const ProfileBadge = ({ user }) => {
  ...
  return !!user ? (
    <div className="flex flex-row gap-6 max-w-xs mx-auto items-center bg-red-500">
      ...
      <div className="flex flex-col gap-1 text-left">
        ...
        <p className="text-sm">{user.location} FWAFWFA</p>
        <p className="text-sm">{getTitle(user.subarea, user.gender)}</p>
        <p className="text-xs">{'WAFWAFAWFWAF'}</p>
      </div>
    </div>
  ) : (
