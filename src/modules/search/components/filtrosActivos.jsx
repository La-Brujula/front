...
export const FiltrosActivos = ({ filters, setFilters }) => {
    const removeFromFilters = useCallback((filterName) => () => {
        const obj = {}
        obj[filterName] = undefined
        setFilters(obj)
    }, [])
    return <div className="flex flex-row flex-wrap gap-4 mt-4 items-center">
        ...
        {!!filters &&
            Object.entries(filters)?.map(([k, v]) => !!v &&
                <p
                    ...
                    onClick={removeFromFilters(k)}><Close /> <b>{k}</b>: {v.toString()}</p>)}
    </div>
